...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    const total = await (prisma as any).clientEnquiry.count();
    const rows = await (prisma as any).clientEnquiry.findMany({
      skip: offset,
      take: limit,
      orderBy: { id: 'desc' }
    });
...
export const getOne = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).clientEnquiry.findUnique({ where: { id } });
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).clientEnquiry.create({ data: formattedData });
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).clientEnquiry.update({
      where: { id },
      data: formattedData
    });
...
export const remove = async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id, 10);
    await (prisma as any).clientEnquiry.delete({ where: { id } });
