...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    const total = await (prisma as any).subcontractorTask.count();
    const rows = await (prisma as any).subcontractorTask.findMany({
      skip: offset,
      take: limit,
      orderBy: { id: 'desc' }
    });
...
export const getOne = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).subcontractorTask.findUnique({ where: { id } });
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).subcontractorTask.create({ data: formattedData });
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    const row = await (prisma as any).subcontractorTask.update({
      where: { id },
      data: formattedData
    });
...
export const remove = async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id, 10);
    await (prisma as any).subcontractorTask.delete({ where: { id } });
